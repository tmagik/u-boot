 ...
 *
 * SiFive sdram-denali.c: init Denali controller registers from device tree
 * 
 * Adapted from coreboot.
 * Adapted from sdram-rk3399.c
 */
...
struct chan_info {
	struct denali_ddr_pctl_regs *pctl;
	struct denali_ddr_pi_regs *pi;
	struct denali_ddr_publ_regs *publ;
	struct denali_msch_regs *msch;
};
...
struct dram_info {
	...
	struct clk ddr_clk;
	struct denali_cru *cru;
	struct denali_pmucru *pmucru;
	struct denali_pmusgrf_regs *pmusgrf;
	struct denali_ddr_cic_regs *cic;
#endif
	struct ram_info info;
	struct denali_pmugrf_regs *pmugrf;
};
...
struct sifive_dmc_plat {
	...
	struct dtd_sifive_denali_dmc dtplat;
#else
	struct denali_sdram_params sdram_params;
#endif
	struct regmap *map;
};
...
static void phy_dll_bypass_set(struct denali_ddr_publ_regs *ddr_publ_regs,
			       u32 freq)
{
...
static void set_memory_map(const struct chan_info *chan, u32 channel,
			   const struct denali_sdram_params *sdram_params)
{
	const struct denali_sdram_channel *sdram_ch =
		    ...
		    const struct denali_sdram_params *sdram_params)
{
	...
	copy_to_reg(&denali_ctl[1], &params_ctl[1],
		    sizeof(struct denali_ddr_pctl_regs) - 4);
	writel(params_ctl[0], &denali_ctl[0]);
	copy_to_reg(denali_pi, &sdram_params->pi_regs.denali_pi[0],
		    sizeof(struct denali_ddr_pi_regs));
			  ...
			  unsigned char channel, u32 ddrconfig)
{
	...
	struct denali_msch_regs *ddr_msch_regs = chan->msch;
			    ...
			    const struct denali_sdram_params *sdram_params)
{
	...
	for (channel = 0, idx = 0;
	     ...
	     channel++) {
		const struct denali_sdram_channel *info =
			&sdram_params->ch[channel];
		struct denali_msch_regs *ddr_msch_regs;
		const struct denali_msch_timings *noc_timing;
		      ...
		      const struct denali_sdram_params *sdram_params)
{
	...
	for (channel = 0; channel < 2; channel++) {
		...
		struct denali_ddr_publ_regs *publ = chan->publ;
...
static int denali_dmc_ofdata_to_platdata(struct udevice *dev)
{
#if !CONFIG_IS_ENABLED(OF_PLATDATA)
...
{
	struct sifive_dmc_plat *plat = dev_get_platdata(dev);
	struct dtd_sifive_denali_dmc *dtplat = &plat->dtplat;
	int ret;
...
static int denali_dmc_init(struct udevice *dev)
{
	struct dram_info *priv = dev_get_priv(dev);
	struct sifive_dmc_plat *plat = dev_get_platdata(dev);
	int ret;
#if !CONFIG_IS_ENABLED(OF_PLATDATA)
	struct denali_sdram_params *params = &plat->sdram_params;
#else
	struct dtd_sifive_denali_dmc *dtplat = &plat->dtplat;
	struct denali_sdram_params *params =
					(void *)dtplat->sifive_sdram_params;
...
//	priv->pmusgrf = syscon_get_first_range(ROCKCHIP_SYSCON_PMUSGRF);
	priv->pmucru = sifive_get_pmucru();
	priv->cru = sifive_get_cru();
...
static int denali_dmc_probe(struct udevice *dev)
{
#ifdef CONFIG_SPL_BUILD
	if (denali_dmc_init(dev))
		return 0;
#else
	struct dram_info *priv = dev_get_priv(dev);
...
//	priv->info.size = sifive_sdram_size(
//			(phys_addr_t)&priv->pmugrf->os_reg2);
#endif
	return 0;
}

static int denali_dmc_get_info(struct udevice *dev, struct ram_info *info)
{
	struct dram_info *priv = dev_get_priv(dev);
...
static struct ram_ops denali_dmc_ops = {
	.get_info = denali_dmc_get_info,
};


static const struct udevice_id denali_dmc_ids[] = {
	{ .compatible = "sifive,rk3399-dmc" },
	{ }
};

U_BOOT_DRIVER(dmc_denali) = {
	.name = "sifive_denali_dmc",
	.id = UCLASS_RAM,
	.of_match = denali_dmc_ids,
	.ops = &denali_dmc_ops,
#ifdef CONFIG_SPL_BUILD
	.ofdata_to_platdata = denali_dmc_ofdata_to_platdata,
#endif
	.probe = denali_dmc_probe,
